...
class Aozora2Html
  class Tag
    ...
    class Ruby < ReferenceMentioned
      ...
      def gen_rt(string)
        if string != ""
          %Q|<rt class="real_ruby">#{string}</rt>|
        else
          %Q|<rt class="dummy_ruby"></rt>|
        end
      end

      def to_s
        '<ruby><rb>' + @target.to_s + '</rb><rp>（</rp><rt>'.encode("shift_jis") + @ruby.to_s + '</rt><rp>）</rp></ruby>'.encode("shift_jis")
      end
