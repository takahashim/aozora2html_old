...
class Aozora2Html
  ...
  @@accent_table = loader.load("../data/accent_table.yml")
  # [class, tag]
  @@command_table = loader.load("../data/command_table.yml")
  ...
  def char_type (char)
    if char.is_a?(Tag::Accent)
      :hankaku
    elsif char.is_a?(Tag::Gaiji)
      :kanji
    elsif char.is_a?(Tag::Kunten) # just remove this line
      :else
    elsif char.is_a?(Tag::DakutenKatakana)
      :katakana
    elsif char.is_a?(Tag)
      :else
      ...
      :katakana
    elsif char.match(/[０-９Ａ-Ｚａ-ｚΑ-Ωα-ωА-Яа-я�ｯ＆’，．]/)
      :zenkaku
  ...
  def buf_is_blank? (buf)
    buf.each do |token|
      if token.is_a?(String) and token != ""
        return false
      elsif token.is_a?(Tag::OnelineIndent)
        return :inline
      end
    end
    true
  ...
  def general_output
    if @style_stack.last
      raise Aozora2Html::Error, "#{@style_stack.last[0]}中に改行されました。改行をまたぐ要素にはブロック表記を用いてください"
    end
    ...
    buf.each do |s|
      if s.is_a?(Tag::OnelineIndent)
        tail.unshift(s.close_tag)
      elsif s.is_a?(Tag::UnEmbedGaiji) and not(s.escaped?)
        # 消してあった※を復活させて
        @out.print "※"
      elsif s.is_a?(Tag::MultilineChitsuki)
  ...
  def search_front_reference (string)
    ...
    elsif last_string.is_a?(Tag::ReferenceMentioned)
...
#      if @ruby_buf_protected
  ...
  def kuten2png (substring)
    ...
    if (match and not(desc.match(/非0213外字/)) and not(desc.match(/※.*※/)))
      ...
      Tag::EmbedGaiji.new(self, folder,code,desc.gsub!("＃",""))
     else
       substring
     end
  end
  ...
  def escape_gaiji (command)
    ...
    Tag::UnEmbedGaiji.new(self, command)
  end
  ...
  def dispatch_aozora_command
    if @stream.peek_char(0) != "＃"
      ...
      # avoid to try complex ruby -- escape to notes
        ...
        Tag::Kaeriten.new(self, command)
      elsif command.match(/^（(.+)）$/)
        Tag::Okurigana.new(self, command.gsub!(/[（）]/,""))
  ...
  def apply_jisage (command)
    if command.match(/まで/) or command.match(/終わり/)
      ...
      if command.match(/この行/)
        # 1行だけ
        @buffer.unshift(Tag::OnelineJisage.new(self, jisage_width(command)))
        nil
      else
        if @buffer.length == 0 and @stream.peek_char(0) == "\r\n"
          ...
          Tag::MultilineJisage.new(self, jisage_width(command))
        else
          @buffer.unshift(Tag::OnelineJisage.new(self, jisage_width(command)))
          nil
        end
  ...
  def apply_chitsuki (string, multiline = false)
    if string.match(/ここで地付き終わり/) or
      ...
      if multiline
        ...
        Tag::MultilineChitsuki.new(self, l)
      else
        # 1行のみ
        Tag::OnelineChitsuki.new(self, l)
      end
    end
  end
  ...
  def exec_inline_end_command (command)
    ...
    if encount == "本文"
      ...
      raise Aozora2Html::Error, "#{encount}を終了しようとしましたが、#{@style_stack.last[0]}中です"
    end
  end
  ...
  def exec_block_start_command (command)
    ...
    if command.match(/太字/)
      ...
      push_block_tag(Tag::MultilineStyle.new(self, "futoji"),match)
    ...
    if command.match(/斜体/)
      ...
      push_block_tag(Tag::MultilineStyle.new(self, "shatai"),match)
    ...
    if command.match(/(.*)段階(..)な文字/)
      ...
      push_block_tag(Tag::FontSize.new(self,
  ...
  def exec_img_command (command,raw)
    ...
    if match
      ...
      Tag::Img.new(self, src, css_class, alt, width, height)
    else
      apply_rest_notes(command)
    end
  end
  ...
  def include_ruby? (array)
    array.index{|elt|
      if elt.is_a?(Tag::Ruby)
        true
      elsif elt.is_a?(Tag::ReferenceMentioned)
        if elt.target.is_a?(Array)
          include_ruby?(elt.target)
        else
          elt.target.is_a?(Tag::Ruby)
        end
      end
    }
  end
  ...
  def rearrange_ruby_tag (targets, upper_ruby, under_ruby = "")
    ...
    Tag::Ruby.new(self, target,upper,under)
  end
  ...
  def rearrange_ruby (targets, upper_ruby, under_ruby = "")
    if include_ruby?(targets)
      ...
      targets.each{|x|
        ...
        elsif x.is_a?(Tag::ReferenceMentioned)
          if x.target.is_a?(Array)
            # recursive
  ...
  def exec_style (targets, command)
    ...
    if try_kuten != command
      ...
      Tag::Dir.new(self, targets)
    elsif command.match(/横組み/)
      Tag::InlineYokogumi.new(self, targets)
    elsif command.match(/罫囲み/)
      Tag::InlineKeigakomi.new(self, targets)
    elsif command.match(/キャプション/)
      Tag::InlineCaption.new(self, targets)
    elsif command.match(/返り点/)
      Tag::Kaeriten.new(self, targets)
    elsif command.match(/訓点送り仮名/)
      Tag::Okurigana.new(self, targets)
    elsif command.match(/見出し/)
      midashi_type = :normal
      if command.match(/同行/)
        midashi_type = :dogyo
      ...
      Tag::Midashi.new(self, targets, command, midashi_type)
    elsif command.match(/(.*)段階(..)な文字/)
      whole, nest, style = command.match(/(.*)段階(..)な文字/).to_a
      Tag::InlineFontSize.new(self,targets,
                              convert_japanese_number(nest).to_i,
                              if style.match("小")
                                :sho
                              else
      ...
      if targets.length == 1 and targets[0].is_a?(Tag::Ruby)
        tag = targets[0]
        if tag.under_ruby == ""
      ...
      if found
        ...
        Tag::Decorate.new(self, targets, filter.call(found[0]), found[1])
      else
        nil
      end
    end
  ...
  def apply_dakuten_katakana (command)
    ...
    if found = search_front_reference(frontref)
      Tag::DakutenKatakana.new(self, n,found.join)
    else
      apply_rest_notes(command)
    end
  end
  ...
  def apply_rest_notes (command)
    ...
    Tag::EditorNote.new(self, command)
  end
  ...
  def apply_ruby
    ...
    notes = []
    @ruby_buf.each{|token|
      if token.is_a?(Tag::UnEmbedGaiji)
        ans.concat("※")
        token.escape!
        notes.push(token)
      else
    ...
    @buffer.push(Tag::Ruby.new(self, ans, ruby))
    @buffer = @buffer + notes
    @ruby_buf = [""]
    nil
  end
